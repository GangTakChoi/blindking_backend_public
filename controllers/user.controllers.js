...
exports.addUser = async function (req, res, next) {
  try {
    ...
    tempResult = await userModel.findOne(
    ...
    console.log(`CRYPTO_SALT::${process.env.CRYPTO_SALT}`);
    console.log(
      `CRYPTO_REPETITION_NUMBER::${process.env.CRYPTO_REPETITION_NUMBER}`
    );
    console.log(`CRYPTO_KEY_LEN::${process.env.CRYPTO_KEY_LEN}`);
    console.log(`CRYPTO_ALGORITHM::${process.env.CRYPTO_ALGORITHM}`);
    console.log(`CRYPTO_ENCODING::${process.env.CRYPTO_ENCODING}`);
...
exports.useMatchingTopDisplay = async (req, res, next) => {
  try {
    ...
    if (matchingTopDisplayUseingTime === null) {
      ...
      res.status(200).json({
        matchingTopDisplayUseingTime:
          updatedUserInfo.matchingTopDisplayUseingTime.getTime(),
      });
      return;
    }
    ...
    if (timeDiffTime < fourHoursTime) {
      ...
      let displayDiffSec = Math.floor(
        displayTime / 1000 - Math.floor(timeDiffMin) * 60
      );
...
exports.putActiveStatus = async (req, res, next) => {
  try {
    ...
    let myUserInfo = await userModel.findOne(
      { _id: myObjectId },
      { roleName: 1 }
    );
    ...
    if (stopPrieod === "3일") {
      ...
      activeStopPrieodLastDate.setMonth(
        activeStopPrieodLastDate.getMonth() + 1
      );
    } else if (stopPrieod === "3개월") {
      activeStopPrieodLastDate.setMonth(
        activeStopPrieodLastDate.getMonth() + 3
      );
    } else if (stopPrieod === "6개월") {
      activeStopPrieodLastDate.setMonth(
        activeStopPrieodLastDate.getMonth() + 6
      );
    } else if (stopPrieod === "영구정지") {
      activeStopPrieodLastDate.setFullYear(
        activeStopPrieodLastDate.getFullYear() + 80
      );
    } else {
      next(createError(400, "invalid request"));
      return;
...
exports.activeMatching = async (req, res, next) => {
  try {
    ...
    if (!userInfo.isActiveMatching) {
      ...
      if (
        userInfo.mbti === "unkown" ||
        !userInfo.birthYear ||
        !userInfo.region ||
        !userInfo.region.rootAreaCode ||
        !userInfo.region.subAreaCode
      ) {
        next(
          createError(
            400,
            "자기소개작성에서 MBTI, 출생년도, 지역을 설정하셔야\n매칭활성화가 가능합니다."
          )
        );
        return;
      }
...
exports.createToken = async function (req, res, next) {
  try {
    ...
    const encryptedUserPw = await crypto
      .pbkdf2Sync(
        userPw,
        process.env.CRYPTO_SALT,
        Number(process.env.CRYPTO_REPETITION_NUMBER),
    ...
    let resUserInfo = {
      ...
      nickname: userInfo.nickname,
      matchingTopDisplayUseingTime:
        userInfo.matchingTopDisplayUseingTime.getTime(),
    };
...
exports.setSelfIntroduction = async function (req, res, next) {
  try {
    ...
    let resultCount = await questionListModel.countDocuments({
    ...
    if (
      isNaN(userBirthYear) ||
      userBirthYear < 1900 ||
      userBirthYear > fullAgeBirthYear
    ) {
      next(createError(400, "invalid request"));
      return;
    }
    ...
    // 지역 정보
    if (
      req.body.rootAreaCode !== undefined &&
      req.body.subAreaCode !== undefined
    ) {
      ...
      if (
        !areaInfoList ||
        !Array.isArray(areaInfoList) ||
        areaInfoList.length !== 2
      ) {
        next(createError(400, "invalid request"));
        return;
      }
...
exports.getMypageInfo = async function (req, res, next) {
  try {
    ...
    let typeList =
      req.query.type !== undefined ? req.query.type.split(",") : [];
    let limit = 10;
    ...
    if (isCommentInfo) {
      ...
      boardCommentList = await boardCommentModel.aggregate([
        {
          $match: {
            writerUserId: mongoose.Types.ObjectId(myObjectId),
            isDelete: false,
          },
        },
        {
...
exports.getSelfIntroduction = async function (req, res, next) {
  try {
    ...
    let questionList = await questionListModel
      .find(
        { isShow: true, isDelete: false },
        { order: 1, content: 1, inputType: 1 }
      )
      .sort({ order: 1 });
    let upperArea = await areaModel.find(
      { depth: 0 },
      { _id: 0, createdAt: 0, updatedAt: 0 }
    );
    let subArea = await areaModel
      .find({ depth: 1 }, { _id: 0, createdAt: 0, updatedAt: 0 })
      .sort({ name: 1 });
    let mbtiInfo = await commonModel.findOne({ key: "mbti" });
...
exports.getMachingPartnerList = async function (req, res, next) {
  try {
    ...
    let searchMbtiList = [];
    if (
      typeof req.query.mbtiList === "string" &&
      req.query.mbtiList.length > 0
    ) {
    ...
    if (
      typeof req.query.upperAreaCodeList === "string" &&
      req.query.upperAreaCodeList.length > 0
    ) {
    ...
    if (
      typeof req.query.subAreaCodeList === "string" &&
      req.query.subAreaCodeList.length > 0
    ) {
    ...
    if (!userInfo.isActiveMatching) {
      next(
        createError(400, "매칭이 활성화된 상태에서만 인연찾기가 가능합니다.")
      );
      return;
    }
    ...
    if (isInitial) {
      ...
      let subArea = await areaModel
        ...
        .sort({ name: 1 });
      let questionList = await questionListModel
        .find({ isShow: true, isDelete: false }, { content: 1 })
        .sort({ order: 1 });
    ...
    if (result !== null) {
      ...
      if (status === "reject") {
        ...
        if (result.status === "accept") {
          ...
          await establisFriendRelation(
            myObjectId,
            friendObjectId,
            result.chattingRoomId
          );
...
exports.getFriendInfoList = async function (req, res, next) {
  try {
    ...
    let friendInfoList = await userFriendsModel
      .find(filter)
    ...
    friendInfoList.forEach((friendInfo) => {
      ...
      if (friendInfo.status === "accept") {
        ...
        friendInfo.chattingRoomId.messageUnReadInfos.forEach(
          (messageUnreadInfo) => {
...
exports.reportUser = async function (req, res, next) {
  try {
    ...
    let reportedUserInfo = await userModel.findOne(
      { _id: reportedUserId },
      { nickname: 1 }
    );
...
exports.checkDuplicateId = async function (req, res, next) {
  try {
    ...
    let userInfo = await userModel.findOne({ id: checkingId }, { id: 1 });
...
exports.getReportList = async function (req, res, next) {
  try {
    ...
    let reportType =
      req.query.reportType === undefined ? null : req.query.reportType;
    let reportTarget =
      req.query.reportTarget === undefined ? null : req.query.reportTarget;
    let reporterNickname =
      req.query.reporter === undefined ? null : req.query.reporter;
    let reportedNickname =
      req.query.reported === undefined ? null : req.query.reported;
    ...
    if (reportedNickname !== null)
      filter.reportedUserNickname = reportedNickname;

    let reportList = await userReportModel
      .find(filter)
      .skip(skip)
      .limit(limit)
      .sort({ _id: -1 });
...
exports.getChattingInfo = async function (req, res, next) {
  try {
    ...
    let userFriendInfo = await userFriendsModel.findOne({
      userObjectId: userId,
      friendObjectId: friendObjectId,
    });
    ...
    let chattingInfo = await chattingRoomModel.findOne(
      { _id: userFriendInfo.chattingRoomId },
      { messageRecords: 1 }
    );
    ...
    res
      .status(200)
      .json({ result: "success", chattingList: chattingInfo.messageRecords });
  } catch (error) {
    console.log(error);
    next(createError(500, "server error"));
  }
...
exports.getCommentListForOne = async function (req, res, next) {
  try {
    ...
    let commentList = await boardCommentModel
      .find({ boardId: boardId, writerUserId: userId })
      .sort({ _id: -1 });
  ...
  try {
    ...
    let userInfo = await userModel.aggregate([
      ...
      { $sort: { "activeStopHistoryList._id": -1 } },
      {
        $group: {
          _id: "$_id",
          activeStopHsty: { $push: "$activeStopHistoryList" },
        },
      },
      { $project: { activeStopHistoryList: "$activeStopHsty" } },
    ]);

    let activeStopHistoryList =
      userInfo.length < 1 ? [] : userInfo[0].activeStopHistoryList;

    res
      .status(200)
      .json({
        result: "success",
        activeStopHistoryList: activeStopHistoryList,
      });
  } catch (error) {
    console.log(error);
    next(createError(500, "server error"));
...
async function establisFriendRelation(
  myObjectId,
  friendObjectId,
  chattingRoomId = null
) {
